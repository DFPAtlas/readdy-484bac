...
export default function ClientLogin() {
  ...
  function traceLog(msg: string, data?: Record<string, unknown>) {
    ...
    try {
      if (process.env.NODE_ENV !== 'production') {
        console.log(`[ClientLogin] ${msg}`, data || '');
      }
    } catch {}
  }
  ...
  const handleSocialLogin = async (provider: 'google' | 'apple' | 'linkedin_oidc') => {
    try {
      ...
      sessionStorage.setItem('oauth_callback_intent', JSON.stringify({ type: 'client', isNewUser: false }));

      const redirectTo = `${window.location.origin}/auth/callback`;
      console.log('[OAuth] Client login redirectTo:', redirectTo);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!clientData) {
        traceLog('no client record, signing out');
      ...
      traceLog('client found', { profile_completed: clientData.profile_completed });
      ...
      const target = isMobile ? '/client/mobile' : '/client/dashboard';
      traceLog('redirecting to dashboard', { target, isMobile });
      router.push(target);
    } catch (err: any) {
      traceLog('login failed', { error: err.message });
  ...
  return (
    <div className="min-h-screen flex bg-[#071321] relative">
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-12 lg:px-12 relative overflow-hidden">
        ...
        <LoginFormCard userTypeLabel="Returning User" formId="client-login-form">
          {error && (
          ...
          <form id="client-login-form" onSubmit={handleSubmit} className="space-y-5">
            <div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Link href="/client/forgot-password" className="text-sm font-medium transition-colors hover:text-[#3B82F6]" style={{ color: "#1DA1F2" }}>
                Forgot password?
              </Link>
            </div>
          ...
          <div className="mt-4 text-center pt-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <p className="text-xs text-[#AAB7C4]/40">
              Registered with a magic link? Use <Link href="/client/forgot-password" className="transition-colors hover:text-[#3B82F6]" style={{ color: "#1DA1F2" }}>Forgot Password</Link> to set your own password.
            </p>
          </div>

          <div className="mt-5 text-center">
            <Link href="/guard/login" className="text-sm text-[#AAB7C4]/60 hover:text-white transition-colors">
              Are you a guard? <span className="font-medium" style={{ color: "#1DA1F2" }}>Login here</span>
            </Link>
          </div>
