...
export default function ClientRewardsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (codeRes.data) {
        ...
        const { data: nameRes } = await supabase.from('clients').select('contact_name,company_name').eq('user_id', user.id).maybeSingle();
        const namePart = (nameRes?.contact_name || nameRes?.company_name || '').replace(/[^A-Za-z0-9]/g, '').toUpperCase().substring(0, 12);
        ...
        const { data: insertData } = await supabase.from('qg_referral_codes').insert({ owner_user_id: user.id, owner_role: 'client', code: newCode, status: 'active' }).select('code').single();
      ...
      if (refsRes.data) {
        ...
        const clicks = refsRes.data.filter((r: any) => ['clicked','profile_started','account_created','verified','approved'].includes(r.status)).length;
        const started = refsRes.data.filter((r: any) => ['profile_started','account_created','verified','approved'].includes(r.status)).length;
        const verified = refsRes.data.filter((r: any) => ['verified','approved'].includes(r.status)).length;
  ...
  const shareEmail = () => {
    ...
    const body = encodeURIComponent(`Hi! I'm using QuickGuard for security staffing and thought you'd be interested. Create a verified guard or client profile and we both earn QG Tokens (discount credits that reduce your platform costs).\n\n${link}\n\n100 QG Tokens = £10 QuickGuard credit. Reduce your subscription and job posting costs.`);
    window.open(`mailto:?subject=${subject}&body=${body}`);
  };

  const shareWhatsApp = () => {
    const link = `${window.location.origin}/qg-launch-rewards?ref=${referralCode}`;
    const text = encodeURIComponent(`Join QuickGuard — QG Launch Rewards! Create a verified profile and earn QG Tokens towards platform discounts.\n\n${link}`);
  ...
  const handleSendInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInviteForm({ recipient_name: '', recipient_email: '', recipient_role: 'client', message_optional: '' });
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Refer businesses and guards — reduce your platform costs</p>
          </div>
          <Link href="/client/dashboard" className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-slate-300 rounded-lg hover:bg-slate-700 transition-colors text-sm whitespace-nowrap cursor-pointer">
        ...
        {/* Balance Card */}
        <div className="bg-gradient-to-br from-teal-500/20 to-blue-500/10 border border-teal-500/20 rounded-2xl p-8 mb-6">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/client/payment-centre" className="px-6 py-3 bg-teal-500 text-slate-900 font-bold rounded-xl hover:bg-teal-400 transition-all text-sm whitespace-nowrap cursor-pointer">
              Use on Subscription
            </Link>
          </div>
        </div>
          ...
          <div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
              {[
                ...
                { label: 'Profiles Started', value: stats.started, icon: 'ri-building-line' },
            ...
            {preAccountData?.pre_account_tokens_linked && (
              <div className="bg-gradient-to-r from-teal-500/10 to-blue-500/10 border border-teal-500/20 rounded-2xl p-6 mb-6">
            ...
            <div className="bg-[#111d35] border border-[#1a2b4a] rounded-2xl p-6 mb-6">
              <h3 className="text-lg font-bold text-white mb-3">How to use your QG Tokens</h3>
              <p className="text-slate-400 text-sm mb-4">Invite businesses that need security or guards you trust. QG Tokens activate after verified account creation. Use them to reduce subscription costs, job posting fees, and future QuickGuard platform credits.</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="bg-[#0B1933] rounded-xl p-4 border border-[#1a2b4a]">
                  <div className="w-8 h-8 flex items-center justify-center rounded-lg bg-teal-500/10 mb-2"><i className="ri-store-2-line text-teal-400"></i></div>
                  <p className="text-white text-sm font-semibold">Refer Businesses</p>
                  <p className="text-slate-500 text-xs mt-1">Businesses that need security earn you 500 tokens each.</p>
                </div>
                <div className="bg-[#0B1933] rounded-xl p-4 border border-[#1a2b4a]">
                  <div className="w-8 h-8 flex items-center justify-center rounded-lg bg-teal-500/10 mb-2"><i className="ri-shield-user-line text-teal-400"></i></div>
                  <p className="text-white text-sm font-semibold">Refer Guards</p>
                  <p className="text-slate-500 text-xs mt-1">Verified guards you refer earn you 250 tokens each.</p>
                </div>
                <div className="bg-[#0B1933] rounded-xl p-4 border border-[#1a2b4a]">
                  <div className="w-8 h-8 flex items-center justify-center rounded-lg bg-teal-500/10 mb-2"><i className="ri-discount-percent-line text-teal-400"></i></div>
                  <p className="text-white text-sm font-semibold">Reduce Costs</p>
                  <p className="text-slate-500 text-xs mt-1">Use tokens as discount credits on your subscription and job postings.</p>
                </div>
              </div>
            </div>
        ...
        {tab === 'invite' && (
          <div>
            <div className="bg-[#111d35] border border-[#1a2b4a] rounded-2xl p-6 mb-8">
              ...
              <p className="text-slate-400 text-sm mb-6">Invite businesses that need security or guards you trust. QG Tokens activate after verified account creation.</p>
          ...
          <div>
            {ledger.length === 0 ? (
              <div className="bg-[#111d35] border border-[#1a2b4a] rounded-2xl p-12 text-center">
                ...
                <p className="text-slate-400">No token activity yet. Earn tokens by referring verified businesses and guards.</p>
              </div>
            ) : (
              <div className="bg-[#111d35] border border-[#1a2b4a] rounded-2xl overflow-hidden">
                <div className="overflow-x-auto">
          ...
          <div>
            {redemptions.length === 0 ? (
              <div className="bg-[#111d35] border border-[#1a2b4a] rounded-2xl p-12 text-center">
                ...
                <Link href="/client/payment-centre" className="mt-4 inline-block px-6 py-3 bg-teal-500 text-slate-900 font-bold rounded-xl hover:bg-teal-400 transition-all text-sm whitespace-nowrap cursor-pointer">
                  Use Tokens Now
                </Link>
              </div>
            ) : (
