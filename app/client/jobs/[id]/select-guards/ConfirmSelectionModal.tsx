    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#111d35] rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto border border-[#1e2d4d]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6">
          ...
          {isUnderstaffed && (
            <div className="bg-amber-500/10 border border-amber-500/25 rounded-xl p-3 mb-4 flex items-center gap-2">
              <i className="ri-error-warning-line text-amber-400"></i>
              <p className="text-sm text-amber-400">
                You have selected {selectedGuards.length} of {job.number_of_guards} required guards.
              </p>
            </div>
          )}
          {isOverstaffed && (
            <div className="bg-red-500/10 border border-red-500/25 rounded-xl p-3 mb-4 flex items-center gap-2">
              ...
              <p className="text-sm text-red-400">
                You have selected {selectedGuards.length} guards but only {job.number_of_guards} are required.
              </p>
            </div>
          )}
              ...
              <div key={guard.id} className="flex items-center gap-3 bg-[#162036] border border-[#1e2d4d] rounded-xl p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    {guard.sia_verified && (
                      <span className="text-xs text-emerald-400"><i className="ri-shield-check-line mr-0.5"></i>SIA</span>
                    )}
                    {guard.sia_licence_number && (
                      <span className="text-xs text-slate-500">{guard.sia_licence_number}</span>
                    )}
                    {guard.rating && (
