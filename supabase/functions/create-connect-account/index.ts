...
serve(async (req) => {
  ...
  try {
    ...
    const { data: guard } = await supabase
      .from('guards')
      .select('id, stripe_account_id, email, full_name, stripe_customer_id, stripe_connect_status')
      .eq('user_id', user.id)
      .maybeSingle();
    ...
    const refreshUrl = `${siteUrl}/guard/bank-settings?connect=refresh`;
    const returnUrl = `${siteUrl}/guard/bank-settings?connect=success`;
