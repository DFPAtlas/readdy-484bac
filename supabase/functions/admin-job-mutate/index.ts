...
serve(async (req) => {
  ...
  try {
        ...
        if (clientEmail) {
          await fetch(`${supabaseUrl}/functions/v1/send-job-deleted-email`, {
            ...
            body: JSON.stringify({
              ...
              job_id: jobId,
              deletion_reason: 'This job posting has been removed by the administrator.',
            }),
          });
        }
      } catch (emailErr: any) {
    ...
    if (action === 'flag') {
      ...
      return new Response(JSON.stringify({ success: true, message: 'Job flagged' }), {
        status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (action === 'unflag') {
      ...
      return new Response(JSON.stringify({ success: true, message: 'Flag removed' }), {
        status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (action === 'accept_applicant') {
      ...
      if (existingAssign) {
        return new Response(JSON.stringify({ error: 'Guard is already assigned to this job' }), {
      ...
      const { error: assignErr } = await supabase.from('job_assignments').upsert({
        job_id: jobId, guard_id: guardId, status: 'confirmed', assigned_at: now,
      ...
      return new Response(JSON.stringify({ success: true, message: 'Application accepted' }), {
        status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (action === 'decline_applicant') {
      ...
      return new Response(JSON.stringify({ success: true, message: 'Application declined' }), {
        status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (action === 'bulk') {
      ...
      if (bulkAction === 'delete') {
        updatePayload = { is_deleted: true, deleted_at: now };
        actionType = 'bulk_job_deleted';
        actionDesc = `Bulk deleted ${ids.length} jobs`;
      } else if (bulkAction === 'close') {
        updatePayload.status = 'completed';
        actionType = 'bulk_job_closed';
        actionDesc = `Bulk closed ${ids.length} jobs`;
      } else if (bulkAction === 'pause') {
        updatePayload.status = 'paused';
        actionType = 'bulk_job_paused';
        actionDesc = `Bulk paused ${ids.length} jobs`;
      } else if (bulkAction === 'open') {
        updatePayload.status = 'open';
        actionType = 'bulk_job_reopened';
        actionDesc = `Bulk reopened ${ids.length} jobs`;
      } else if (bulkAction === 'flag') {
        updatePayload.risk_level = 'suspicious';
        actionType = 'bulk_job_flagged';
        actionDesc = `Bulk flagged ${ids.length} jobs`;
      } else {
      ...
      return new Response(JSON.stringify({ success: true, message: `${ids.length} jobs ${bulkAction}ed` }), {
        status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (action === 'update_payment_status') {
      ...
      await supabase.from('payment_audit_logs').insert({
        ...
        reason: reason || `Admin ${action} from ${oldStatus} to ${newPaymentStatus}`,
      });
