...
serve(async (req) => {
  ...
  try {
    const { client_email, client_name, job_title, amount, status, message } = await req.json();
    ...
    const renderRes = await fetch(`${supabaseUrl}/functions/v1/render-email-template`, {
      ...
      body: JSON.stringify({ template_slug: 'payment_notification', to: client_email, variables, from: 'QuickGuard <billing@quickguard.uk>' }),
    });
