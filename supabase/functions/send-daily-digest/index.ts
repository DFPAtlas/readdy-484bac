...
serve(async (req) => {
  ...
  try {
    const { guard_email, guard_name, digest_date, digest_content } = await req.json();
    ...
    await fetch(`${supabaseUrl}/functions/v1/render-email-template`, {
      ...
      body: JSON.stringify({ template_slug: 'daily_digest', to: guard_email, variables, from: 'QuickGuard <digest@quickguard.uk>' }),
    });
