...
interface SIALicenceImageProps {
  path: string | null;
  label: string;
  className?: string;
}

export default function SIALicenceImage({ path, label, className = '' }: SIALicenceImageProps) {
  ...
  useEffect(() => {
    ...
    async function loadSignedUrl() {
      ...
      setError(false);
      try {
        const url = await getSIALicenceSignedUrl(path as string, 3600);
  ...
  if (loading) {
    return (
      <div className={`bg-gray-100 rounded-lg flex items-center justify-center min-h-[120px] ${className}`}>
  ...
  if (error || !signedUrl) {
    return (
      <div className={`bg-red-50 rounded-lg p-4 min-h-[120px] flex flex-col items-center justify-center text-center ${className}`} role="alert">
        ...
        <p className="text-xs text-red-600 font-medium mb-1">Failed to load image</p>
  ...
  const fileName = path.split('/').pop() || 'sia-licence';
    ...
    return (
      <div className={`bg-gray-50 rounded-lg p-4 ${className}`}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <p className="text-sm font-medium text-gray-900">{label || 'SIA Licence'}</p>
        ...
        <div className="flex flex-wrap gap-2">
          <a
            ...
            aria-label={`View ${label || 'SIA licence'} PDF in new tab`}
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            aria-label={`Download ${label || 'SIA licence'}`}
    ...
    <div className={`bg-gray-50 rounded-lg overflow-hidden ${className}`}>
      ...
      <div className="flex flex-wrap gap-2 p-3">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          aria-label={`Open ${label || 'SIA licence'} full size in new tab`}
          className="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 whitespace-nowrap cursor-pointer"
        >
      ...
      {enlarged && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setEnlarged(false)}>
          <div className="relative max-w-4xl max-h-[90vh] w-full flex items-center justify-center">
            <img src={signedUrl} alt={label} className="max-w-full max-h-[85vh] object-contain" />
            <button
